...
`grid` 是本工具维护的真实挂单网格：下到交易所的是普通 GTC limit 单，不是 Hyperliquid 原生网格。服务器 worker 每分钟检查一次，目标是让买卖两边各保持最多 10 张活跃子单；成交后按原子单提交限价和 `gap` 补下一张反向 limit 单。
...
- worker 补齐每侧子单时，交易所一旦接受提交，本轮就视为已补一个名额；即使 GTC 立即成交，也不会在同一轮为了凑够 10 张活跃单连续追单。
