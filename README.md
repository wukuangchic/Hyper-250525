...
`grid` 现在是有限订单链，不再维持固定数量或固定密度。新建 grid 初始为空，不提交任何格子单。当前观察期冻结所有会增加绝对仓位的 P4 `limit-chase`；只有会减少绝对仓位的 P4 IOC 市价回归单仍可确认成交，并按实际 `avgPx` 在反方向出生 `grid_leg=1` GTC 子单。P10 可在仓位越过 `--limit` 最小边界时，把离盘口最近的一张同方向 active 加风险单上提：撤单前先按原单剩余 size、IOC 最坏价格、当前杠杆和 withdrawable 预算保证金，并复核不会越过最大仓位边界；全部通过后才确认撤掉原单并按相同 size 市价成交，再以 `2 × 实际成交价 - 原挂单价` 挂一张反向 GTC。P10 不增加计划总数量，但会把原本更远的风险提前兑现。near 位于 `2 * gap`；默认 far 不近于 near 与反向最远 active 单的价格中点。实际市价成交量超过 `3 * base_size` 时，依据 `floor(fill_size / base_size)` 扩展为 near、far1、far2……，各 far 依次放在 near 到最远 active 单之间的 `1/N、2/N……(N-1)/N` 处。该方向没有比 near 更远的 active 时，则从成交价的 `3 * gap` 开始逐层向外排列。任一 far 候选价如果与同方向 active/pending 或本次已生成子单的距离不超过 `0.95 * gap`，会每次向远离盘口方向外移 `1 * gap`，直到找到空档。后续成交由 P2 按 `1 ↔ 0` 延续。
...
- `--gap`：P2 往复使用 `1 * gap`。P0 的 IOC 目标数量先取 `max(abs(当前仓位) * 20%, 2 * base_size)`，再以 `abs(当前仓位) * 40%` 封顶；若该 40% 严格小于 `1 * base_size`，当轮不执行 P0。P4 初始减仓会一次计算并 IOC 回到最近的 `--limit` 边界，目标量至少为 `2 * base_size` 且不超过当前仓位；观察期内 P4 加仓被冻结，不构造或提交市价单。确认成交后，默认等分为 near/far；成交量超过 `3 * base_size` 时自动增加分层分母和 far 子单数量。near 使用 `2 * gap`，far 按 near 到反向最远 active 单的中点或 `i/N` 分点生成；没有更远 active 单时从成交价的 `3 * gap` 开始逐层向外排列。near/far 都沿用统一的 `0.95 * gap` 同方向占位间距检查，被占用时继续向外寻找且不因拥挤减少分母。若某个分母会使任一子单低于最小下单金额，会逐层减少分母，直到所有子单可交易；只有 near/far 两层仍不可交易时，才安全退化为一张完整成交量的 near 单。创建时不生成初始挂单。
...
- P4 `limit-chase`：raw deficit `< 0` 且 signed 仓位 value 仍在 `--limit` 之外时检查回归方向。当前观察期开关 `GRID_LIMIT_CHASE_ADD_RISK_FROZEN=True`：凡是会增加绝对仓位的 P4 都在构造市价单之前跳过，并写入 `limit_chase_add_risk_frozen` action audit；不会创建出生意图、不会提交交易所动作、不会生成反向子单或新增债务。会减少绝对仓位的 P4 保持原语义：按当前 signed value 与最近 `--limit` 边界的差额一次性计算目标数量，至少为 `2 * base_size`，封顶到当前仓位并使用 reduce-only，避免穿过零仓；确认成交后，以实际 `avgPx` 为锚点并按真实成交量动态出生 near/far 多层 `grid_leg=1` GTC 单。升级前已经存在的 P4 出生意图、反向单和债务继续按原链处理；冻结期间新空 grid 不会通过 P4 加风险出生。
...
- 每轮严格按 `P0 → P1 → P2 → P3 → P4 → P5 → P6 → P7 → P8 → P9 → P10` 执行。P0/P1/P2 必须扫描；P3/P4 仅在 raw deficit `< 0` 时执行；P5 仅在 raw deficit `< -100` 时执行；P6 只受自己的过渡余额条件控制；P7 只受自己的债务重组条件控制；P8 无余额和 action-limit 门槛，只做本地累计与入队；P9 在重新读取的 withdrawable 严格 `< 1` 时执行；P10 每账户每轮最多上提一张已有 active 单，并要求撤单、市价单、反向单至少预留 3 次 action headroom，保证金覆盖率严格大于 `1.1`。
...
- OID 从 open orders 消失时，worker 先结合最近成交和订单状态判定：确认全部或部分成交都走 P2；只有确认零成交取消才由 P5 排入恢复队列；尚不能确认则保留记录等待下一轮，避免把延迟成交误判为撤单。
