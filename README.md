...
- 单边漂移接近爆仓时会主动泄压：空仓用 `(liqPx - mid) / (mid - 最近active买入减仓价)`，多仓用 `(mid - liqPx) / (最近active卖出减仓价 - mid)` 计算 `panic_ratio`；若低于 `panic_ratio_threshold`（默认 `20`），Worker 会用 IOC + 滑点保护市价 reduce-only 减一单对应方向的 grid base size，不把这笔减仓作为 grid fill 回补。
