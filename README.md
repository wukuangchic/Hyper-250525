...
- worker 补齐每侧子单时，交易所一旦接受提交，本轮就视为已补一个名额；即使 GTC 立即成交，也不会在同一轮为了凑够 5 张活跃单连续追单。
...
- 如果一侧最靠近市场的单已经离盘口超过约 5 个 gap，worker 会尝试在盘口外侧 3、4 个 gap 各补一张，并按成功提交数量撤掉同侧最远的旧单；不补 5 个 gap，避免与原有档位距离过近。每张仍需通过仓位上限、保证金保护、reduce-only 容量和间距检查，最终保持每边最多 5 张。
