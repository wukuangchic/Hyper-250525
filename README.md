...
`grid` 现在是有限订单链，不再维持固定数量或固定密度。新建 grid 初始为空，不提交任何格子单。P4 `limit-chase` 只保留减少绝对仓位的 IOC 市价回归单，并按实际 `avgPx` 在反方向出生 `grid_leg=1` GTC 子单；P4 不再具有任何加风险路径。P10 可在仓位越过 `--limit` 最小边界时，把离盘口最近的一张同方向 active 加风险单上提：如果原单与当前 mid 的相对距离严格小于 `1 * gap`，则不执行 P10；否则在撤单前按“当前 withdrawable + 原单价值 ÷ 当前杠杆”估算撤单后可用保证金，并同时预算原单剩余 size 的 IOC 最坏成交保证金和反向单可能开出反向仓位所需的保证金，同时复核不会越过最大仓位边界；总覆盖率严格大于 `1.1` 才确认撤掉原单。撤单成功后立即重新读取交易所真实 withdrawable，并按同一份双重预算再次要求覆盖率严格大于 `1.1`，否则不提交市价单，原单进入 P3 恢复队列。市价成交后再读取一次 withdrawable；足够覆盖实际反向单时，以 `2 × 实际成交价 - 原挂单价` 挂一张强制非 reduce-only 的反向 GTC，允许穿过零仓并开出反向仓位；不足时先作为 P3 债务等待。该反向单带 `p10_never_cancel`，程序不会通过 P1、P7 或 P9 撤掉它，交易所异常撤单后也返回 P3，并在它成交前锁住同一经济链的下一次 P10。P10 要求该分支最近一次净 size 回到 0 时的累计已实现盈余严格大于 0；不再要求该盈余完全覆盖本次暂时追价成本。新 P0/P4 链、历史无法追溯链和尚未实现正盈余的链按 0 处理；旧格式 ID 只要经济账本已确认正盈余也可参与。反向 GTC 与实际市价成交之间的最小距离取配置 gap 和“账户实际 taker 费率 + maker 费率 + 一个最小价格跳动”两者较大值，避免完整配对后被手续费吃成负数。P10 每个账户每轮每币种最多执行一次，不增加计划总数量，但会把原本更远的风险提前兑现。near 位于 `2 * gap`；默认 far 不近于 near 与反向最远 active 单的价格中点。实际市价成交量超过 `3 * base_size` 时，依据 `floor(fill_size / base_size)` 扩展为 near、far1、far2……，各 far 依次放在 near 到最远 active 单之间的 `1/N、2/N……(N-1)/N` 处。该方向没有比 near 更远的 active 时，则从成交价的 `3 * gap` 开始逐层向外排列。任一 far 候选价如果与同方向 active/pending 或本次已生成子单的距离不超过 `0.95 * gap`，会每次向远离盘口方向外移 `1 * gap`，直到找到空档。后续成交由 P2 按 `1 ↔ 0` 延续。
...
每张格子单同时持久化 `iteration`。P0/P4 出生的 near/far 首批 GTC 单都从 `0` 开始；此后每次真实提交 ALO 都加 `1`。若 ALO 因 post-only 被拒并外移后重挂，外移重挂几次就继续累加几次。`grid --query` 的订单明细通过 `iter` 列显示该值；升级前已有单兼容记为 `0`。
...
- `--gap`：P2 往复使用 `1 * gap`。P0 的 IOC 目标数量先取 `max(abs(当前仓位) * 20%, 2 * base_size)`，再以 `abs(当前仓位) * 40%` 封顶；若该 40% 严格小于 `1 * base_size`，当轮不执行 P0。P4 只做减仓：一次计算并 IOC 回到最近的 `--limit` 边界，目标量至少为 `2 * base_size` 且不超过当前仓位，不存在 P4 加仓路径。确认成交后，默认等分为 near/far；成交量超过 `3 * base_size` 时自动增加分层分母和 far 子单数量。near 使用 `2 * gap`，far 按 near 到反向最远 active 单的中点或 `i/N` 分点生成；没有更远 active 单时从成交价的 `3 * gap` 开始逐层向外排列。near/far 都沿用统一的 `0.95 * gap` 同方向占位间距检查，被占用时继续向外寻找且不因拥挤减少分母。若某个分母会使任一子单低于最小下单金额，会逐层减少分母，直到所有子单可交易；只有 near/far 两层仍不可交易时，才安全退化为一张完整成交量的 near 单。创建时不生成初始挂单。
...
- P0 `panic`：沿用 `panic_ratio` 触发条件。IOC reduce-only 市价减仓目标量先取 `abs(当前仓位) * 20%` 与 `2 * base_size` 的较大值，再以 `abs(当前仓位) * 40%` 封顶。若 40% 仓位严格小于一张经最小名义金额校正后的 `base_size`，当轮直接跳过 P0，避免逐仓小仓位被最小单量反复 panic 平仓。确认成交后，以实际 `avgPx` 为锚点出生动态分层的反向 `grid_leg=1` GTC 单。空 grid 没有可用于计算 ratio 的 active 减仓格时，P0 不会凭空出生订单。
...
- P5 `anomaly`：只在 raw deficit `< -100` 时运行。记录中的 OID 异常消失后，只有找到同一 OID 的成交记录，才确认 partial fill：`grid_leg=0` 按实际成交量交给 P2，`grid_leg=1` 的已成交部分终结、剩余部分按原挂单价格进入 P8。仅凭交易所状态里的 `origSz > sz` 不足以证明成交，因为订单可能先被原地改单缩小；若找不到同一 OID 的成交记录，则按未成交整单处理，`grid_leg=1` 保留本地原始 size，以新的 FIFO seq 加入 P3 队尾，`grid_leg=0` 直接从 batch 移除。P5→P3 只重置 `p3_queue_seq`，保留原有 `economic_chain_id`。
...
- 每轮严格按 `P0 → P1 → P2 → P3 → P4 → P5 → P6 → P7 → P8 → P9 → P10` 执行。P0/P1/P2 必须扫描；P3/P4 仅在 raw deficit `< 0` 时执行；P5 仅在 raw deficit `< -100` 时执行；P6 只受自己的过渡余额条件控制；P7 只受自己的债务重组条件控制；P8 无余额和 action-limit 门槛，只做本地累计与入队；P9 在重新读取的 withdrawable 严格 `< 1` 时执行；P10 每账户每轮每币种最多上提一张已有 active 单，原单与 mid 距离严格 `< 1 * gap` 时跳过，可追溯的单链累计已实现盈余必须严格 `> 0`，并要求撤单、市价单、非 reduce-only 反向单至少预留 3 次 action headroom，市价追单与反向开仓的合计保证金覆盖率严格大于 `1.1`。
...
- P1 撤单、P6 恢复和 P7 重组都以账户为配额单位，跨 DEX 合并计算；P3 本身按债务队列逐项处理，不额外占用 P1/P6/P7 的账户级配额。P7 每轮最多操作一组远端 BUY/SELL。
...
- OID 从 open orders 消失时，worker 先结合同 OID 的最近成交和订单状态判定：确认全部成交走 P2；确认同 OID 部分成交才走 P2/P8；只有没有同 OID 成交记录的取消才由 P5 按整单排入恢复队列。仅有 `origSz` 与 `sz` 的差异不作为成交证据，避免把原地缩量改单误判为 partial fill。
- P5 只把零成交取消的 leg 1 重新加入 P3 队尾，并分配新的 `p3_queue_seq`，不沿用该订单历史上的 seq；已经完成往复的 leg 0 直接清除，不再重建。
