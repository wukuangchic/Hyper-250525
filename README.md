...
旧任务中保存的默认每侧 5 张会由 worker 自动迁移为每侧 10 张；每轮每侧最多提交 2 张的安全限流不变，因此会分多轮逐步补齐。
...
- 每轮每侧最多向交易所提交 2 张新 grid 单；任一新单若 GTC 立即成交，该侧本轮立刻停止后续补单、成交对向单和历史档位恢复。
...
- 如果一侧最靠近市场的单已经离盘口超过约 5 个 gap，worker 会尝试在盘口外侧 3、4 个 gap 各补一张，并按成功提交数量撤掉同侧最远的旧单；不补 5 个 gap，避免与原有档位距离过近。每张仍需通过仓位上限、保证金保护、reduce-only 容量和间距检查，最终保持每边最多 10 张。
