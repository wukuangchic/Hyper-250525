...
- `--gap`：P2 往复使用 `1 * gap`。P0 的 IOC 目标数量先取 `max(abs(当前仓位) * 20%, 2 * base_size)`，再以 `abs(当前仓位) * 40%` 封顶；若该 40% 严格小于 `1 * base_size`，当轮不执行 P0。P4 初始减仓会一次计算并 IOC 回到最近的 `--limit` 边界，目标量至少为 `2 * base_size` 且不超过当前仓位；P4 加仓仍使用 `2 * base_size`。确认成交后，默认等分为 near/far；成交量超过 `3 * base_size` 时自动增加分层分母和 far 子单数量。near 使用 `2 * gap`，far 按 near 到反向最远 active 单的中点或 `i/N` 分点生成；没有更远 active 单时从成交价的 `3 * gap` 开始逐层向外排列。near/far 都沿用统一的 `0.95 * gap` 同方向占位间距检查，被占用时继续向外寻找且不因拥挤减少分母。若某个分母会使任一子单低于最小下单金额，会逐层减少分母，直到所有子单可交易；只有 near/far 两层仍不可交易时，才安全退化为一张完整成交量的 near 单。创建时不生成初始挂单。
...
- P4 `limit-chase`：raw deficit `< 0` 且 signed 仓位 value 仍在 `--limit` 之外时提交一张 IOC 市价回归单。初始减仓模式按当前 signed value 与最近 `--limit` 边界的差额一次性计算目标数量，至少为 `2 * base_size`，并封顶到当前仓位、使用 reduce-only，避免穿过零仓；加仓模式仍使用 `2 * base_size`。市价纯减仓不受 `withdrawable` 限制；会加仓时，按 IOC 滑点保护价计算最坏名义金额，再除以当前仓位杠杆（空仓使用标的 `maxLeverage`）得到预估保证金占用，只有 `withdrawable / 预估保证金占用 > 1.1` 才放行。确认成交后，以实际 `avgPx` 为锚点并按真实成交量动态出生 near/far 多层 `grid_leg=1` GTC 单。P4 不设每轮全局数量上限，随机顺序中每个符合条件的 Grid 都可以执行；新空 grid 仍仅由 P4 出生。
