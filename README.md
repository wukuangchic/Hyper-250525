...
`grid` 是本工具维护的真实挂单网格：下到交易所的是普通 ALO limit 单，不是 Hyperliquid 原生网格。服务器 worker 每分钟检查一次，目标是让买卖两边各保持最多 10 张活跃子单；成交后按原子单提交限价和 `gap` 补下一张反向 limit 单。
...
- `--avg 200`：把 200 设为目标持仓价值，并与 `--trend` 互斥。仅当某侧活跃单不足 10 张、Worker 向远侧补档时，回归侧保持基础 `gap` 并把单量放大到最高 `1.62` 倍；加仓侧保持基础单量并把补档 `gap` 放大到最高 `1.62` 倍。越过上下限后仍封顶 `1.62`。
...
- 成交反向单始终使用基础 `gap`；若其方向属于当前回归侧，则单量也使用回归倍率，属于加仓侧时仍使用基础单量。近侧补单、paused 恢复和创建时的初始网格使用基础参数，且不会仅因仓位变化撤掉旧挂单。
- `--min 20`：每张子单价值至少 20；不填时按交易所最小名义价值。
- `--total` 和旧 `--max` 不再用于 grid；如果写了会直接报错。
- grid 子单使用 ALO，目标是只挂 maker 单；网格档位仍按提交限价推进，不因实际成交价更优而漂移。
- 成交反向单如果 ALO 因会立即成交被拒，会先确认同侧前后 `0.95 * gap` 内没有已有活跃 grid 挂单；若太近，会优先插入外移方向上间距大于 `1.95 * gap` 的相邻两格中间，否则再向远离盘口方向移动一个 `gap`，最多 20 次。其他 grid 子单 ALO 被拒时只跳过本轮，不换价重试。
- worker 补齐每侧子单时，交易所一旦接受提交，本轮就视为已补一个名额；即使订单随后很快成交，也不会在同一轮为了凑够 10 张活跃单连续追单。
- 每轮每侧最多向交易所提交 1 张新 grid 单；成交对向单、近侧补单、常规补档和历史档位恢复共用这一额度。
- 每侧提交额度的优先级固定为：成交对向单 > 近侧补单 > 常规补档 > paused 恢复；存在待处理成交对向单时，近侧补单不会抢占该侧额度。
...
- 如果一侧最靠近市场的单已经离盘口超过 10 个 gap，worker 只会尝试在盘口外侧 4 个 gap 补一张，并在成功提交后撤掉同侧最远的一张旧单。新单仍需通过仓位上限、保证金保护、reduce-only 容量和间距检查，最终保持每边最多 10 张。
