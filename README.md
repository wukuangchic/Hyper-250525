...
- 成交反向单如果 ALO 因会立即成交被拒，会先确认同侧前后 `0.95 * gap` 内没有已有活跃或可恢复 paused grid 档位；若太近，会优先插入外移方向上间距大于 `1.95 * gap` 的相邻两格中间，否则再向远离盘口方向移动一个 `gap`，最多 20 次。其他 grid 子单 ALO 被拒时只跳过本轮，不换价重试。
