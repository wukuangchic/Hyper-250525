...
- P2 `replacement`：处理确认成交的 active 格子单，以实际成交价为锚点在反方向 `1 * gap` 提交 ALO。每次成交都翻转格属性 `0 ↔ 1`；提交前按当前仓位重新判断增仓或减仓，减仓统一 reduce-only。衍生单提交前按实际成交量和衍生价格预判订单价值；严格 `< 10 USDC` 时终结该段成交链，不提交、也不进入 P3，等于 `10 USDC` 时仍允许提交。
