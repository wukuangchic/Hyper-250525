...
- P4 `limit-chase`：raw deficit `< 0` 且 signed 仓位 value 仍在 `--limit` 之外时，按原方向逻辑提交一张目标数量为 `2 * base_size` 的 IOC 市价回归单；当方向正在减仓时，数量封顶到当前仓位，避免一次动作穿过零仓。市价纯减仓不受 `withdrawable` 限制；会加仓的动作仍要求 `withdrawable > 5`。确认成交后，以实际 `avgPx` 为锚点出生 near/far 两张各半成交量的 `grid_leg=1` GTC 单。P4 不设每轮全局数量上限，随机顺序中每个符合条件的 Grid 都可以执行；新空 grid 仍仅由 P4 出生。
...
- P6 `legacy-pause`：仅用于过渡。升级时现有 active 统一记为 `grid_leg=0`，现有 paused 统一转为 `legacy_pause + grid_leg=1`。每个账户（跨 DEX 合并计算）每轮只恢复一张相对盘口最近的 legacy pause，不受 `withdrawable` 门槛限制；恢复后进入普通 P3 债务队列。
- P7 `debt-restructure`：仅在 `raw deficit < -100` 且 `withdrawable < 5` 时运行；每个账户（跨 DEX、跨币种合并）每轮最多操作一组。只要该币种存在 active `grid_leg=1` 的 BUY 和 SELL，就选择两侧最远的一对（不筛选 reduce-only），撤销两腿后按原跨度围绕当前 mid 对称规划；size 取两腿平均值。两腿都撤销成功后，新单以 `chain_debt` 写入，下一轮进入普通 P3 队列；若只有一腿撤销成功，则把已撤销的一腿按原属性恢复为 P3 债务。网络错误和部分撤单结果会持久化意图并在后续轮次重试。
